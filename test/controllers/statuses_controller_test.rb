...
class StatusesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create status" do
    assert_difference('Status.count') do
      post statuses_url, params: { status: { comment: @status.comment, status: @status.status } }
    end
  ...
  test "should update status" do
    patch status_url(@status), params: { status: { comment: @status.comment, status: @status.status } }
